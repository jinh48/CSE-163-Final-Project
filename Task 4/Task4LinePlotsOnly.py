...
def percentage_missing_data(df):
    ...
    missing data and will print it out.
    '''    
    count_na = df['INCOME'].isna().sum();
    result = count_na / len(df) *  100;
    print('Missing Data: ' + str(round(result, 2)) + "%")    
...
def income_line_plots(df):
    ...
    middle_class = df[(income_value >= 45000) & 
                               (income_value < 150000)]
...
def main():
   ...
   lines plots.
   '''
   burden_data = pd.read_csv('~/Desktop/CSE/CSE 163/project/part 2/Task 4/' +
        'Burden.csv')
   
   percentage_missing_data(burden_data)
