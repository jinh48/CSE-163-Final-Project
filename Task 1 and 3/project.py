...
def problem1a(df):
    ...
    g = sns.catplot(x="Hour", y="Crime Number", hue="Year", data=df1, kind='bar', height=6, aspect=5)
...
def problem1b1(df):
    ...
    """
    top_crimes = df.groupby(['Offense Parent Group']).size().nlargest(10, keep='all').index
    ...
    g = sns.lineplot(x="Year", y="Crime Number", hue="Offense Parent Group", data=df)
...
def problem1b2(df):
    ...
    plt.figure(figsize=(10, 10))
    g = sns.lineplot(x="Year and Month", y="Crime Number", hue="Offense Parent Group", data=df)
...
def problem1c(df):
    ...
    beats = gpd.read_file('C:\\Users\\18223\\Desktop\\cse163_project\\SPD_BEATS_WGS84\\SPD_BEATS_WGS84.shp')
...
def problem3a(force):
    ...
    graphs are produced in submethods, and the last graph is produced in this method.
    Extracts time information from dataset and creates dummy variables for furture uses.
    The first three graphs use the following ratio: percentage of the given group's
    level 2,3 cases in a given year to percentage of the given group's level 1 cases in a
    given year. For example, the ratio for women in 2015 is: 
    (2015 women level 2,3 cases / 2015 level 2,3 cases) / (2015 women level 1 cases / 2015 level 1 cases)
    Supposedly, an umbiased result is that all the ratios are near 1.
    prints the length of dataset and the number of cases where 'gender' is registered as
    'Not Specified' to explain the abnormal 'Not specified' line in plot produced in
    problem3a_helper.
    prints the number of cases related to each race to explain the abnormality in plot produced
    in problem3a_helper. A considerable amount of cases register the race as 'Not Specified',
    leaving the old formula used unreliable. The first attempt to solve is to plot the same graph
    without race as 'Not Specified'. The second attempt is to change involved groups to
    only four main races recorded (Asian, Black or African American, Hispanic or Latino, White)
    and simplify the ratio to:
    number of level 2,3 cases of the given group in a given year to number of level 1 cases of
    the given group in the given year. 
    Supposedly an umbiased result is that all the ratios are similar.
    """
    force = force[force['ID'].str[0:4] != '2020'].copy()
    force = force[['Incident_Type', 'Occured_date_time', 'Subject_Race', 'Subject_Gender']]
    ...
    problem3a_helper('Subject_Race', force[(force['Subject_Race'] != 'Not Specified')], 1)
    
    force = force[force['Subject_Race'].isin(['Black or African American', 'Hispanic or Latino', 'White', 'Asian'])].copy()
    ...
    force_final = force_1_1.merge(force_23_1, how='left')
    force_final['Percentage'] = force_final['Crime'] / force_final['Crime_Total']
    plt.figure(figsize=(7, 7))
    g = sns.lineplot(x='Year', y='Percentage', hue='Subject_Race', data=force_final)
    fig = g.get_figure()
    fig.savefig('3a.png')
...
def problem3a_helper(key, force, check=0):
    ...
    takes the use of force dataset, and takes an indicator to name different graphs.
...
def problem3b_helper123(force, beats, year):
    ...
    a poilce beat.
    Separates the use of force data by level 1 and 2/3, and sums them up by beat.
    Calculates the ratio between two parts and relates the beat names with spatial data.
    """
    force = force.copy()
    ...
    force_23_1 = force_23_1.reset_index()
    force_final = force_1_1.merge(force_23_1, how='inner', left_on='Beat', right_on='Beat')
    force_final['Percentage'] = force_final['Sum23'] / force_final['Sum1']
    merged = beats.merge(force_final, left_on='BEAT', right_on='Beat', how='inner')
    fig, ax = plt.subplots(1)
    beats.plot(color = "#EEEEEE", ax = ax)
...
def main():
    ...
    of every crime reported from 2008 to present, and it reads a SPD Use of Force
    record from 2014 to present.
    Then it processes both files in different methods corresponding to different
    research questions.
    """
    df = pd.read_csv('C:\\Users\\18223\\Desktop\\cse163_project\\SPD_Crime_Data__2008_Present.csv')
    problem1ab(df)
    problem1c(df)
    force = pd.read_csv('C:\\Users\\18223\\Desktop\\cse163_project\\Use_Of_Force.csv')
    problem3a(force)
    problem3b(df, force)
