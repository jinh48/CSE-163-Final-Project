...
def problem1ab_t(df):
    ...
    assert_equals(12, len(df['Year'].unique()))   #should be 12 years from 2008 to 2019
    ...
    assert_equals(144, len(df['Year and Month'].unique()))  # should be 12 years * 12 months = 144 year and month
...
def problem1a_t(df):
    ...
    assert_equals(288, len(df1))   #12 years times 24 hours should be 288 rows in df1
...
def problem1b2_t(df):
    ...
    df['Crime Number'] = crimes
    df = df[df['Offense Parent Group'].isin(top_crimes)]
    assert_equals(10, len(df['Offense Parent Group'].unique()))  #10 crime types
    assert_equals(1440, len(df))     #12 years * 12 months * 10 crime types
...
def problem3a_t(force):
    ...
    assert_equals(6, len(force['Year'].unique()))  # should be 6 years from 2014 to 2019
    force['Month'] = new[0].astype(int)
    assert_equals(12, len(force['Month'].unique()))  # should be 12 months
    force['Year and Month'] = new[2].str[0:4] + new[0]
    assert_equals(72, len(force['Year and Month'].unique()))  # should be 6 years * 12 months = 72 year and month
    force['Crime'] = 1
    force['Crime_Total'] = 1
    ...
    assert_equals(41, a)   #should be 6 years * 7 genders = 42. It is 41 though because Pacific Islanders
                           #does not have use of force level 2/3 records in 2014. Shown in the next line.
    assert_equals(41, len(force[force['Incident_Type'] != 'Level 1 - Use of Force'] .groupby(['Subject_Race', 'Year']).size()))   
    
    a = problem3a_helper_t('Subject_Race', force[(force['Subject_Race'] != 'Not Specified')])
    assert_equals(35, a)   #should be 6 years * 6 genders = 36. Again due to missing record it is 35.
    ...
    assert_equals(24, len(force_final))  #should be 6 years * 4 selected races = 24
...
def problem3b_t(df, force):
    ...
    assert_equals(48, a)   #Supposedly, length of police beats with use of force data is 51,
    assert_equals(53, b)   #for there are 51 police beats. But a is smaller because not every police beat
    assert_equals(49, c)   #has both Lvl1 and Lvl2,3 information, as displayed by b and c.
    problem3b_helper0_t(df, beats, 2019)
    a, b, c = problem3b_helper123_t(force, beats, '2019')
    assert_equals(45, a)   #Similarly, only 45 police beats have the required ratio because
    assert_equals(54, b)   #54 police beats(more than 51 existed due to recording error) have Lvl 1 information,
    assert_equals(49, c)   #only 49 police beats have Lvl 2,3 information.
